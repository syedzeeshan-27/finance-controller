...
A perfect score proves nothing on its own; a controlled gap does. The naive baseline and this engine run on the same worlds, through the same parsers, the same loader and the same grader. The only thing that differs between them is the matching logic, so the entire gap between their scores is attributable to that logic and nothing else. That is the one thing a bare 100% can never tell you, and it is why the baseline sits in every report.

And the gap is wide. The first script anyone writes recovers 79.2% of reconciliation matches, gets 51.3% of tax labels right, forces all 30 ambiguous cases into wrong answers, and claims ₹1,65,084 of input credit it is not entitled to. On the identical inputs the engine recovers everything, forces nothing, and claims ₹0 falsely, and an independent verifier that shares no code with it signs off each run. The 100% is just where the engine's line tops out.

Read the 100% itself narrowly: it means the engine saturates our synthetic test set, the undecidable cases included, where saturating means abstaining. It is not a claim that real bank or GST data would reconcile at 100%; real-world accuracy was never measured, and what was never measured at all (real Razorpay settlement data, real GST and TDS data, production volumes) is listed in the README under [What is real and what is synthetic](README.md#what-is-real-and-what-is-synthetic).
