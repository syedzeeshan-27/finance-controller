...
**A perfect score proves nothing on its own; the gap to the baseline does.** The naive matcher (amount within ₹1, value date within 3 days, first match wins, no references) runs on the same data through the same grader, so the only thing that differs between the two is the matching logic. It recovers 79.2% of matches, forces all 30 ambiguous cases into wrong answers, and claims ₹1,65,084 of tax credit falsely; on the identical inputs the engine recovers everything, forces nothing, and claims ₹0. The 100% just marks where the engine tops out, saturating a synthetic set with the abstain cases included, not real data, which was never measured. Full treatment: [Reading the 100% honestly](ARCHITECTURE.md#reading-the-100-honestly); scope: [What is real and what is synthetic](#what-is-real-and-what-is-synthetic).
